...
const ParticleTextEffect: React.FC<ParticleTextEffectProps> = ({
    ...
    const dottify = () => {
        ...
        const filteredPixels = pixels.filter(p => p.rgb[3] && !(p.x % particleDensity) && !(p.y % particleDensity));

        ctx.clearRect(0, 0, canvas.width, canvas.height);

        filteredPixels.forEach((p, i) => {
        ...
        particlesRef.current.splice(filteredPixels.length, particlesRef.current.length);
    };
    ...
    const write = () => {
        ...
        textBox.str = text;
        const fontSizeBasedOnWidth = (canvas.width * 1.2) / textBox.str.length;
        textBox.h = Math.floor(Math.min(canvas.height * 0.9, fontSizeBasedOnWidth));
        ...
        colors.forEach((c, i) => gradient.addColorStop(i / N || 0, `#${c}`));
    ...
    const animateWrapper = () => {
        ...
        animationIdRef.current = requestAnimationFrame(animateWrapper);
    };
    ...
    const initialize = () => {
        ...
        // Use a high resolution container logic
