...
export default function FocusTab() {
    ...
    const statsStrip = (
        <div className="flex-1 grid grid-cols-3 gap-2 lg:gap-4 min-h-0 w-full shrink-0">
            <div className="bg-white/[0.03] border border-white/[0.06] backdrop-blur-xl rounded-2xl p-3 lg:p-5 flex flex-col justify-between relative overflow-hidden group hover:bg-white/[0.04] transition-all">
                ...
                <div className="flex items-center justify-between relative z-10">
                    ...
                    <div className="w-6 h-6 lg:w-8 lg:h-8 rounded-full flex items-center justify-center" style={{ background: "var(--color-accent-glow)" }}>
            ...
            <div className="bg-white/[0.03] border border-white/[0.06] backdrop-blur-xl rounded-2xl p-3 lg:p-5 flex flex-col justify-between relative overflow-hidden group hover:bg-white/[0.04] transition-all">
                ...
                <div className="flex items-center justify-between relative z-10">
                    ...
                    <div className="w-6 h-6 lg:w-8 lg:h-8 rounded-full bg-[#4ADE80]/10 flex items-center justify-center">
            ...
            <div className="bg-white/[0.03] border border-white/[0.06] backdrop-blur-xl rounded-2xl p-3 lg:p-5 flex flex-col justify-between relative overflow-hidden group hover:bg-white/[0.04] transition-all">
                ...
                <div className="flex items-center justify-between relative z-10">
                    ...
                    <div className="w-6 h-6 lg:w-8 lg:h-8 rounded-full bg-[#FBBF24]/10 flex items-center justify-center">
    ...
    return (
        <div className="flex flex-col lg:flex-row h-full w-full gap-4 animate-fade-in p-1 overflow-y-auto lg:overflow-visible snap-y snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            <AnimatePresence>
                {timerAlert && (
            ...
            <div className="flex flex-col lg:flex-[2] w-full min-h-full lg:min-h-0 h-full lg:h-auto snap-start snap-always shrink-0 gap-4 pb-2 lg:pb-0">
                ...
                <div className="flex-[3] flex flex-col items-center justify-center bg-white/[0.03] border border-white/[0.06] backdrop-blur-xl rounded-2xl p-6 relative">
                    ...
                    <div className="absolute top-4 right-4 flex gap-1 p-1 rounded-xl bg-white/[0.03] border border-white/[0.05] z-10">
                        <button
                    ...
                    {/* Timer Ring */}
                    <div className="relative flex-1 flex flex-col items-center justify-center min-h-[250px] lg:min-h-[280px] w-full mt-8 lg:mt-0">
                        <div
                            className="w-[240px] h-[240px] sm:w-[280px] sm:h-[280px] rounded-full flex flex-col items-center justify-center transition-all duration-300 relative mx-auto"
                            style={{
                        ...
                        >
                            ...
                            <span className="text-5xl sm:text-6xl font-black font-mono tracking-tighter text-white drop-shadow-md">
                                {mode === "pomodoro" && isRunning ? formatTime(displayTime) : formatTime(elapsed)}
                            </span>
                            {isPaused && (
                                <span className="absolute bottom-12 sm:bottom-16 text-xs font-bold text-white/30 tracking-widest uppercase">PAUSED</span>
                            )}
                        </div>
                    </div>

                    {/* Timer Controls */}
                    <div className="flex items-center justify-center gap-4 mt-6 lg:mt-4 h-16 z-10 shrink-0">
                        {!isRunning ? (
                            <button
                                onClick={handleStart}
                                className="w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-[1.05] active:scale-95 bg-white text-black shadow-[0_0_30px_rgba(255,255,255,0.3)]"
                            >
                            ...
                            <>
                                <button
                                    ...
                                    className="w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center transition-all hover:scale-105 active:scale-95 bg-white/[0.08] hover:bg-white/[0.12] border border-white/[0.1]"
                                >
                                    {isPaused ? <Play fill="currentColor" size={20} className="text-white ml-1" /> : <Pause fill="currentColor" size={20} className="text-white" />}
                                </button>
                                <button
                                    onClick={stopSession}
                                    className="w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center transition-all hover:scale-105 active:scale-95 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30"
                                >
                ...
                {/* Pomodoro Settings (Only visible when not running) */}
                {mode === "pomodoro" && !isRunning && (
                    <motion.div
            ...
            <div className="flex flex-col lg:flex-[1] w-full min-h-full lg:min-h-0 h-full lg:h-auto snap-start snap-always shrink-0 gap-4 pt-2 lg:pt-0">
